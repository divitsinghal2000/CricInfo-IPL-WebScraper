    ...
    for (let i = 0; i < scorecardElem.length; i++) {
        ...
        for (let key = 0; key < playerRows.length; key++) {
            ...
            if (isAllowed) {
                ...
                processPlayer(playerName, MatchDay, runs, balls, sixes, fours, strikeRate, teamName, opponent);
            }
        }
    }
}

function processPlayer(playerName, MatchDay, runs, balls, sixes, fours, strikeRate, teamName, opponent) {
    let playerObject = {
        Name: playerName,
        MatchDay: MatchDay,
        Runs: runs,
    ...
    let teamPath = path.join(__dirname, "STATS", teamName);
